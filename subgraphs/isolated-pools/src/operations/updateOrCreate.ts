...
export const updateOrCreateAccountVToken = (accountAddress: Address, poolAddress: Address, marketAddress: Address, blockNumber: BigInt, enteredMarket: Box<boolean> | null = null): AccountVToken => {
  let enteredMarketBool = false;
  if (enteredMarket !== null) {
    enteredMarketBool = enteredMarket.value;
  }

  const result = getOrCreateAccountVToken(accountAddress, poolAddress, marketAddress, enteredMarketBool);
...
export const updateOrCreateMarketAction = (vTokenAddress: Address, action: i32, pauseState: boolean): MarketAction => {
