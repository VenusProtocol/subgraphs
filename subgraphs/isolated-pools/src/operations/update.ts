...
export const updateAccountVTokenAccrualBlockNumber = (accountAddress: Address, poolAddress: Address, marketAddress: Address, blockNumber: BigInt): AccountVToken => {
...
export const updateAccountVTokenSupply = (accountAddress: Address, poolAddress: Address, marketAddress: Address, blockNumber: BigInt, accountSupplyBalanceMantissa: BigInt): AccountVToken => {
  const accountVToken = updateAccountVTokenAccrualBlockNumber(accountAddress, poolAddress, marketAddress, blockNumber);
...
export const updateAccountVTokenTransferFrom = (accountAddress: Address, poolAddress: Address, marketAddress: Address, blockNumber: BigInt, amount: BigInt, exchangeRate: BigInt): AccountVToken => {
  const amountUnderlyingMantissa = exchangeRate.div(exponentToBigInt(18)).times(amount);

  const accountVToken = updateAccountVTokenAccrualBlockNumber(accountAddress, poolAddress, marketAddress, blockNumber);

  accountVToken.totalUnderlyingRedeemedMantissa = accountVToken.totalUnderlyingRedeemedMantissa.plus(amountUnderlyingMantissa);
  accountVToken.save();
  return accountVToken as AccountVToken;
};

export const updateAccountVTokenTransferTo = (accountAddress: Address, poolAddress: Address, marketAddress: Address, blockNumber: BigInt): AccountVToken => {
  const accountVToken = updateAccountVTokenAccrualBlockNumber(accountAddress, poolAddress, marketAddress, blockNumber);
...
export const updateMarket = (vTokenAddress: Address, blockNumber: i32, blockTimestamp: i32): Market => {
  ...
  const tokenPriceCents = getTokenPriceInCents(Address.fromBytes(market.pool), vTokenAddress, market.underlyingDecimals);
  market.underlyingPriceCentsMantissa = tokenPriceCents;

  market.accrualBlockNumber = valueOrNotAvailableIntIfReverted(marketContract.try_accrualBlockNumber()).toI32();
  market.blockTimestamp = blockTimestamp;

  const exchangeRateMantissa = valueOrNotAvailableIntIfReverted(marketContract.try_exchangeRateStored());
  ...
  market.borrowRateMantissa = valueOrNotAvailableIntIfReverted(marketContract.try_borrowRatePerBlock());
  market.supplyRateMantissa = valueOrNotAvailableIntIfReverted(marketContract.try_supplyRatePerBlock());

  market.totalBorrowsMantissa = valueOrNotAvailableIntIfReverted(marketContract.try_totalBorrows());
  market.totalSupplyVTokenMantissa = valueOrNotAvailableIntIfReverted(marketContract.try_totalSupply());
