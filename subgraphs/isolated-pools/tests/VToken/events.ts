...
export const createBadDebtIncreasedEvent = (vTokenAddress: Address, borrower: Address, badDebtDelta: BigInt, badDebtOld: BigInt, badDebtNew: BigInt): BadDebtIncreasedEvent => {
  ...
  const badDebtDeltaParam = new ethereum.EventParam('badDebtDelta', ethereum.Value.fromUnsignedBigInt(badDebtDelta));
  event.parameters.push(badDebtDeltaParam);

  const badDebtOldParam = new ethereum.EventParam('badDebtOld', ethereum.Value.fromUnsignedBigInt(badDebtOld));
  event.parameters.push(badDebtOldParam);

  const badDebtNewParam = new ethereum.EventParam('badDebtNew', ethereum.Value.fromUnsignedBigInt(badDebtNew));
  event.parameters.push(badDebtNewParam);

  return event;
};

export const createNewAccessControlManagerEvent = (vTokenAddress: Address, oldAccessControlManager: Address, newAccessControlManager: Address): NewAccessControlManagerEvent => {
...
export const createSpreadReservesReducedEvent = (vTokenAddress: Address, benefactor: Address, reduceAmount: BigInt, newTotalReserves: BigInt): SpreadReservesReducedEvent => {
