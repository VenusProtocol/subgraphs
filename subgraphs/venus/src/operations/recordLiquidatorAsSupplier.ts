...
import { VToken as VTokenContract } from '../../generated/Comptroller/VToken';
import { LiquidateBorrow } from '../../generated/templates/VToken/VToken';
...
export const recordLiquidatorAsSupplier = (event: LiquidateBorrow): void => {
  if (event.receipt) {
    const reversedLogs = event.receipt!.logs.reverse();
    reversedLogs.reduce<LiquidatorTransferMarker>((acc, curr, idx) => {
      const protocolSeizeTopic = Bytes.fromByteArray(crypto.keccak256(ByteArray.fromUTF8('LiquidateBorrow(address,address,uint256,address,uint256)')));
      ...
      if (idx == acc.current && acc.current != 0) {
        ...
        // Update balance
        const liquidatorAccountVTokenResult = getOrCreateAccountVToken(liquidator, acc.vTokenAddress);
