...
const checkWeEth = async () => {
  ...
  const supplierBalanceSum = data!.supplierAccounts.reduce((acc: BigInt, curr: { effective_balance: string }) => {
    return BigInt(acc.toString()) + BigInt(curr.effective_balance);
  }, BigInt(0));

  const inRange = (num: bigint) => num >= -10000000000n && num <= 10000000000n;
  assert.ok(inRange(BigInt(data!.tvl!.tvl) - (supplierBalanceSum as bigint)), `TVL: ${data!.tvl!.tvl} supplierBalanceSum: ${supplierBalanceSum}`);
};
