...
export const createFunctionRegistryChangedEvent = (functionSignature: string, active: boolean): FunctionRegistryChangedEvent => {
  const event = changetype<FunctionRegistryChangedEvent>(newMockEvent());

  event.parameters = [];
  const signatureParam = new ethereum.EventParam('signature', ethereum.Value.fromBytes(Bytes.fromByteArray(ByteArray.fromUTF8(functionSignature))));
